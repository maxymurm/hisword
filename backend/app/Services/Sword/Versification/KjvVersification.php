...
/**
 * KJV Versification System.
 *
 * Maps OSIS book IDs to their position within SWORD's canonical ordering
 * and provides verse count data for each chapter. This is used to compute
 * the flat index for looking up verses in SWORD binary index files.
 *
 * SWORD stores OT and NT in separate files. Within each testament,
 * the flat index is computed as:
 *   - Chapter 0 (book intro): 1 slot (verse 0)
 *   - Each chapter c: (maxVerse[c] + 1) slots (verse 0 = heading, then verses 1..n)
 *   - Total slots per book = 1 + sum(maxVerse[c] + 1) for c = 1..maxChapter
 */
class KjvVersification extends AbstractVersification
{
    /**
     * OT books in canonical SWORD order.
     * Index within this array = book index within the OT testament.
     */
    ...
    /**
     * NT books in canonical SWORD order.
     */
     ...
     * The array index is 1-based (chapter 1 = index 0).
     */
    public const VERSE_COUNTS = [
        // OT Books
    ...
    /**
     * OSIS ID → human-readable name mapping.
     */
    ...
    /**
     * Pre-computed cumulative slot offsets per book within each testament.
     * Computed lazily and cached.
     */
    private static ?array $otOffsets = null;
    private static ?array $ntOffsets = null;

    /**
     * Clear cached offsets (useful after verse count corrections or in tests).
     */
    public static function clearOffsetCache(): void
    {
        self::$otOffsets = null;
        self::$ntOffsets = null;
    }
